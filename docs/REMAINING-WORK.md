...
**The photo-scan prompts have never met a live model.** The whole of
`backend/src/services/ai/prompts/nutrition-photo.prompt.ts` — including the rules
that decide whether the model asks "air fried or deep fried?" rather than
something useless — was written without being able to call Gemini. The parser
downstream is defensive and the UI degrades to the plain single-shot flow when a
reply is unusable, so a bad prompt costs accuracy rather than breaking anything.
But whether the questions are GOOD is only knowable by running it. Expect to edit
the wording after seeing real output; that one file is the only thing to change,
and `npm run check:services` exercises both passes against the real API.

**The app icons are placeholders.** `frontend/public/pwa-*.png`,
`apple-touch-icon.png` and `favicon.svg` are generated by
`frontend/scripts/generate-icons.ts` — three ascending bars in the accent colour,
legible and correctly sized, but not a designed logo. Replace the PNGs directly or
edit the mark in that script and re-run `npm run icons --workspace frontend`.
Nothing in CI checks them against `tokens.ts`, deliberately: such a check would
start failing the moment real artwork replaces them.

**The Training tab's segmented control has tabs with no tabpanels.** The three
buttons in `TrainingTab.tsx` carry `role="tab"` and `aria-selected` correctly, but
the content below them is not marked up as `role="tabpanel"` with an
`aria-labelledby` pointing back, so a screen reader announces the tabs without the
relationship to what they switch. Unlike the bottom nav — where the tab roles were
actively wrong and were removed — this is incomplete rather than broken.

**Trend windows are still computed from the server's UTC date.**
`body-composition.service.ts` and `trend-rules.ts` derive their 30-day window from
the server clock. Unlike the daily calorie total, a one-day shift in a 30-day
regression window is immaterial, so this was left alone rather than threading the
caller's date through a second path. Worth knowing before adding a
"this week" view, where it would matter.
